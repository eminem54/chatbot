...
def joined(msg):
    room=session.get('room','')
    join_room(room)
    socketio.emit('slot', {'data': '새마을금고 고객센터에 오신것을 환영합니다. 궁금하신 항목을 선택하거나, 간단한 문장을 입력해주세요.',
                           'slots': ['상품 소개','지점 안내','자주 묻는 키워드','추천 상품']}, room=room)
...
def server_msg_function(msg):
    ...
    if intentData == '고객 상담':
        ...
        #최신으로 자주 묻는 질문을 뽑아낸다.
        socketio.emit('slot',{'data':'최근 검색 순위가 높은 키워드입니다.\n 원하시는 내용을 선택해주세요.','slots':['상상모바일정기적금','주식 계좌 이벤트','체크카드 발급등록']})
